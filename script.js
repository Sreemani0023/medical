...
const appState = {
  ...
  currentPatientUsername: null, bookingFor: 'myself'
};
...
function handleLogin(e){
  ...
  if(appState.userRole==='doctor'){
    let found=false;
    hospitalsData.forEach(h=>{const d=h.doctors.find(doc=>doc.name.toLowerCase().includes(u.toLowerCase()));if(d){found=true;docId=d.id;}});
    if(!found) return toast('Error','Doctor not found in registry','err');
  }
  ...
  if(appState.userRole==='doctor'){document.getElementById('sidebarDoctorName').textContent=u;showPage('doctor-bookings');}
